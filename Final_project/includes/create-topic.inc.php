...
if (isset($_POST['create-topic']))
{
    ...
    {
        $sql = "insert into topics(topic_subject, topic_date, topic_cat, topic_by, rank) "
                . "values (?,now(),?,?,?)";
        ...
        {
            mysqli_stmt_bind_param($stmt, "ssss", $topicSubject, $topicCategory, $_SESSION['userId'], $rank);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_store_result($stmt);
            
            $topicid = mysqli_insert_id($conn);
            
            $sql = "insert into posts(post_content, post_date, post_topic, post_by,rank) "
                    . "values (?,now(),?,?,?)";
            $stmt = mysqli_stmt_init($conn);
            ...
            {
                mysqli_stmt_bind_param($stmt, "ssss", $postContent, $topicid, $_SESSION['userId'],$rank);
                mysqli_stmt_execute($stmt);
                mysqli_stmt_store_result($stmt);
